...
(`002-v2-architecture.md`) that were noted but not resolved. For a future
...
- Docker self-hosting note: since this app has no client-side wallet, likely
  *no* `NEXT_PUBLIC_*` env vars are needed at all (everything Solana-related
...
- Does changing a private game's password affect already-joined players
  (presumably no — it only gates *new* joins), and does this need explicit
  confirmation?
