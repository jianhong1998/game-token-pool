...
Decision record from the `/grill-me` session that sharpened `001-PRD.md` into
...
tracked separately in `003-pending-discussion.md`.
...
`pool_payout(player, amount)` to *any* player in the game, for any amount up
...
game — player wallets *and* any pot/pool token accounts still holding a
...
- `join_game` creates the user's ATA for that game's SPL mint; `quit_game`
  ...
  existence/balance *is* the membership record, avoiding duplicate
...
*game* instead (Game Token Module), and multi-tenancy (the other thing
