...
- [ ] Single-recipient `transfer_token` instruction (fixed accounts — sender, sender ATA, recipient, recipient ATA, token program; no dynamic recipient list) moves tokens between two players' ATAs for the same game (per `002-v2-architecture.md` Q14).
