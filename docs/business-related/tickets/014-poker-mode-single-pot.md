...
**What to build:** The core loop of a poker hand with a single pot (no side pots yet) — players contribute or go all-in, admin declares winner(s) at showdown. Betting rounds, turn order, and fold are explicitly out of scope (per `002-v2-architecture.md` Q5) — cards and betting decisions happen at the physical table.
